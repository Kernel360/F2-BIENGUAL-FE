...
export default function QuizAccuracyPanel() {
  ...
  if (isError) return <ErrorPanel title="최근 5주 퀴즈 정답율" />;
  if (!data?.data.questionSummaryList)
    return (
      <EmptyPanel title="최근 5주 퀴즈 정답율" message="데이터가 없습니다." />
    );

  const chartData = processQuizAccuracyData(data.data.questionSummaryList);
